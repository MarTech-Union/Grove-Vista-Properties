...
export default function AdminSidebar({ collapsed, onToggle }) {
  ...
  return (
    <aside
      className={`flex flex-col bg-slate-900 text-white transition-all duration-200 ease-in-out ${
        collapsed ? "w-16" : "w-60"
      } shrink-0`}
    >
      {/* Logo / brand */}
      <div className={`flex h-16 items-center border-b border-white/10 ${collapsed ? "justify-center px-0" : "px-5"}`}>
        {collapsed ? (
          ...
          <span className="text-base font-black tracking-tight">
            ...
            <span className="ml-2 rounded-md bg-blue-600 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wider text-white">Admin</span>
          </span>
        )}
      </div>
